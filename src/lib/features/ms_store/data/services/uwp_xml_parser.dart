...
class const UwpXmlParser() {
      ...
      if (updates != null) {
        for (final XmlElement updateElement in updates.findElements('Update')) {
          ...
          for (final XmlElement fileElement in filesElement.findElements('File')) {
            ...
            // Skip encrypted files. File type starts with "e" means encrypted (e.g. .eappx).
            // App cannot install encrypted files. Only plain appx/msix are usable.
            // Also skip files without packageFullName. These are metadata cabs (Abm_*.cab) not real packages.
            // Example: Abm_...cab has no InstallerSpecificIdentifier.
  ...
  // Store always gives SHA1 as Digest and SHA256 as AdditionalDigest.
  // SHA256 is stronger, so use it for file check. Keep SHA1 for URL lookup.
  // If no SHA256, fallback to first AdditionalDigest so check still works.
  ...
  // Get download URL from FE3 response.
  // If many files, match by digest (SHA1). Use SHA1, not SHA256.
  // FE3 FileDigest is SHA1. If digest is null or no match, use first Url.
